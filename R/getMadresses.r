...
########################################################
###grab Madison Addresses from parcel shapefile
########################################################
...
MadParcels <- as.data.frame(subset(data, Property_5 == "Madison"))
#nrow(MadParcels)

MadParcels[is.na(MadParcels)] <- ""

head(MadParcels)

names(MadParcels) <- c("hn","sd","sn","ss","au","c", "GIS_ID")

unique(MadParcels$ss)

test <- subset(MadParcels, sn == "WASHINGTON")

write.csv(MadParcels,"./Maddresses.csv", row.names=F, na='', quote=FALSE) 

########################################################
###bring in Madison energy data
########################################################
