...
## energy.table.list <- rep(list(NULL),nrow(Active_Address_List))
...
energy.table.list <- rep(list(NULL),n)
names(energy.table.list) <- as.character(Active_Address_List$Parcel)[nvec]
...
## for( i in 1:nrow(Active_Address_List) ){
  ...
  energy.table.list[[j]] <- energy_table
  j <- j + 1
