...
## names(energy.table.list) <- as.character(Active_Address_List$Parcel)[44440:(44440+n)]
names(energy.table.list) <- as.character(Active_Address_List$Parcel)

saveRDS(energy.table.list, file = paste("~/Dropbox/Vargo/data/energyTables/energyTableList-", timestamp, ".rds", sep = ""))
...
```{r energyFile}
energy.df <- reshapeEnergyTableList(parsedEnergyList, filename = paste("~/Dropbox/Vargo/data/energyTables/energyFile-",timestamp,".csv",sep=""))
```

```{r merge}
parcelData <- read.csv(file = "~/Dropbox/Vargo/data/energyTables/Assessor_Property_Information.csv", stringsAsFactors = FALSE)

dataFull <- merge(energy.df,parcelData,by.x="parcel",by.y="Parcel")
dataFull[1,]
```
